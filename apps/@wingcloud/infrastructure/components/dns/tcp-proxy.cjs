// This file is imported fron the "node-tcp-proxy" package and was slighltly modified
...
module.exports.startServer = function() {
  ...
  const proxies = []
  app.get("/", (req, res) => {
    const proxyPort = req.query.fromPort
    const serviceHost = req.query.host
    const servicePort = req.query.toPort
    const proxy = module.exports.createProxy(proxyPort, serviceHost, servicePort);
    proxies.push(proxy);
    res.sendStatus(200);
  });
  const port = Math.floor(Math.random() * 1000 + 4000);
  const server = app.listen(port, () => {
    console.log(`DNS proxy server is running: http://localhost:${port}`);
  });
...
module.exports.createProxy = function(proxyPort,
    ...
    let proxy = new TcpProxy(proxyPort, serviceHost, servicePort, options);
    return proxy;
};
