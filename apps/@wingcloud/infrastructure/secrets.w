...
pub class Secrets {
  ...
  pub inflight create(options: CreateSecretOptions): Secret {
    ...
    if error.contains("ConditionalCheckFailed") {
      throw httpError.HttpError.forbidden("Secret '{options.name}' already exists for the environment '{options.environmentType}'");
    } else {
  ...
  pub inflight get(options: GetSecretOptions): Secret {
    ...
    throw httpError.HttpError.notFound("Secret '{options.id}' not found");
  }
