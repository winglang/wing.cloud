...
export const startReverseProxyServer = (props: ReverseProxyServerProps): number => {
    for (const origin of props.origins) {
        app.all(origin.pathPattern, (req, res) => {
            proxy.web(req, res, {target: !/^https?:\/\//i.test(origin.domainName) ? 'https://' + origin.domainName : origin.domainName})
        });
    }
