...
const targetOptions = process.env["WING_TARGET"] == "tf-aws" ? {
  ...
  bucketClass: "Bucket",
  arguments: [{ topics: {} }, { withTrace: async ({ message, activity }: { message: string, activity: () => Promise<any> }) => {
    console.log(message);
    return activity();
  }}]
};

const BucketInflight = require(join(dirname(require.resolve("@winglang/sdk")), `${targetOptions.directory}/bucket.inflight`));
const bucketName = process.env["LOGS_BUCKET_NAME"];
if (!bucketName) {
  throw new Error("missing bucket name");
}
const logsBucket = new BucketInflight[targetOptions.bucketClass](...targetOptions.arguments);
